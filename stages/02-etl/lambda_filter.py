...
def lambda_handler(event, context):
    ...
    print(f"[INFO] Initial record count: {original_count}")
        ...
        for col, (min_v, max_v) in f["range"].items():
            if col in df.columns:
                ...
                if max_v is not None:
                    df = df[df[col] <= max_v]
                print(f"[INFO] Filtered {col}, removed {before - len(df)} rows outside range")
    ...
    if f.get("timestamp_check", False) and "timestamp" in df.columns:
        ...
        print(f"[INFO] Removed {before - len(df)} future timestamps")
    ...
    print(f"[INFO] Cleaning completed for dataset: {dataset_name}")
